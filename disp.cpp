...
int GzInitDisplay(GzDisplay	*display)
{
	...
	for (int yRes = 0; yRes < display->yres; yRes++) {
		for (int xRes = 0; xRes < display->xres; xRes++) {
			display->fbuf[ARRAY(xRes, yRes)].red = 0;
			display->fbuf[ARRAY(xRes, yRes)].blue = 0;
			display->fbuf[ARRAY(xRes, yRes)].green = 0;
...
int GzGetDisplay(GzDisplay *display, int i, int j, GzIntensity *r, GzIntensity *g, GzIntensity *b, GzIntensity *a, GzDepth *z)
{
	...
	if (i < 0 || i > MAXXRES || j < 0 || j > MAXYRES) {
		return GZ_FAILURE;
	}
	...
	*z = display->fbuf[ARRAY(i, j)].alpha;
	return GZ_SUCCESS;
}
